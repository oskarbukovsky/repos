            ...
            try
            {
                while (true)
                {
                    ...
                    if (tmp != null)
                    {
                        ...
                        {
                            KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().Alive = !tmp.TerminationMessage;
                            KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().AvailablePorts = tmp.AvailablePorts;
                            KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().LastIpAddress = tmp.IpAddress;
                            if (KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().IpAddressHistory.Where(iphistory => iphistory == tmp.IpAddress).Count() == 0)
                            {
                                KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().IpAddressHistory.Add(tmp.IpAddress);
                            };
                            if (!tmp.TerminationMessage)
                            {
                                KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().Tokens.Add(tmp.Token);
                                if (KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().Tokens.Count() > 128)
                                {
                                    KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().Tokens.RemoveAt(0);
                                }
                                if (KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().ValidTokens.First() == "0")
                                {
                                    KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().ValidTokens.RemoveAt(0);
                                }
                                KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().ValidTokens.Add(tmp.Token);
                                if (KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().ValidTokens.Count() > 3)
                                {
                                    KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().ValidTokens.RemoveAt(0);
                                }
                            }
                            else
                            {
                                ...
                                KnownHosts.Host.Where(host => host.Hostname == tmp.Hostname).First().ValidTokens = new List<string> { "0" };
                            }
                        }
            ...
            while (true)
            {
                switch (Console.ReadKey(true).Key)
                {
                    case ConsoleKey.D:
                        ...
                        break;
                    case ConsoleKey.H:
                        foreach (var i in KnownHosts.Host)
                        {
                            Console.WriteLine(i + "\n");
                        }
                        Console.WriteLine("-----------");
